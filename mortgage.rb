...
loop do
  ...
  break if valid_num(loan_amount) && loan_amount.to_f > 0
...
loop do
  ...
  break if valid_integer(loan_duration_year) && loan_duration_year.to_i > 0
  prompt("Invalid number of years. Please input a positive integer.\n")
end

loop do
  prompt("How many months do you have left in the loan? ")
  loan_duration_month = gets.chomp
  break if valid_integer(loan_duration_month) && loan_duration_month.to_i > 0
  prompt("Invalid number of months. Please input a positive integer.\n")
end
